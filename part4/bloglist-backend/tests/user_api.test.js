...
test('fails with a short username', async () => {
  ...
  assert.match(response.body.error, /Username and password must be at least 3 characters long/i)
...
test('fails if username already exists', async () => {
  const newUser = { username: 'testuser', name: 'Test User', password: 'password' }
