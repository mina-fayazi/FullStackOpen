...
test('unique identifier property of blog posts is named id', async () => {
  ...
  response.body.forEach((blog) => {
    assert.strictEqual(
      typeof blog.id,
      'string',
      'Blog id should be defined and a string'
    )
...
test('a valid blog post can be added', async () => {
  ...
  assert.strictEqual(
    ...
    'Blog count should increase by one'
  )
  ...
  assert(
    ...
    'New blog title should be present in the database'
  )
})
...
test('if likes property is missing, it defaults to 0', async () => {
  ...
  assert.strictEqual(
    response.body.likes,
    0,
    'Default likes should be 0 when not provided'
  )
  ...
  assert.strictEqual(
    savedBlog.likes,
    0,
    'Default likes should be 0 in database'
  )
})
...
test('blog without title or url returns 400 Bad Request', async () => {
  ...
  assert.strictEqual(
    ...
    'Invalid blogs should not be added'
  )
})
...
test('update a blog by id (update likes)', async () => {
  ...
  assert.strictEqual(
    ...
    updatedLikes,
    'Likes in the database should be updated'
  )
})
...
test('delete a blog by id', async () => {
  ...
  assert(
    ...
    'Deleted blog should not be in the database'
  )
})
...
test('blogs include user information', async () => {
  ...
  response.body.forEach((blog) => {
    ...
    assert.strictEqual(
      typeof blog.user,
      'object',
      'User field should be an object'
    )
...
test('a valid blog post is assigned a user', async () => {
  ...
  assert.strictEqual(
    savedBlog.user.username,
    users[0].username,
    'Blog should be assigned to an existing user'
  )
})
...
test('requesting blogs without token fails with 401 Unauthorized', async () => {
